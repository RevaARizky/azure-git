...
const initPropertyOverviewEXP = (el) => {
    ...
    const bgItems = bgWrapper.querySelectorAll('.background-item')
    ...
    const tl = gsap.timeline({
        scrollTrigger: {
            ...
            start: () => {
                ...
                if(window.innerWidth < 1024) return `top top`
                return `top top+=${((window.innerHeight/4) - 32)}`
            },
            end: `+=${bgItems.length * 100}%`,
            pin: true,
            scrub: true,
            pinSpacer: true,
            onEnter: () => {
...
export {initPropertyOverviewEXP}